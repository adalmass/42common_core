...
/*   Updated: 2023/06/22 17:01:29 by aldalmas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_tab_a(t_tabs *tab)
{
	...
	{
		check_operators(tab->tab_a[tab->len_tab_a]);
		handle_zeros(tab->tab_a[tab->len_tab_a]);
		temp = ft_strdup(tab->tab_a[tab->len_tab_a]);
		free(tab->tab_a[tab->len_tab_a]);
		tab->tab_a[tab->len_tab_a] = rewrite_str(temp);
		handle_int_limit(tab->tab_a[tab->len_tab_a]);
		free(temp);
		tab->len_tab_a++;
	}
	handle_doubles(tab);
